...
int hacker_counter_id = 1;
int serfs_counter_id = 2;
int hacker_id_id = 3;
int serfs_id_id = 4;
int unboarded_id = 5;
int has_captain_id = 6;
...
/*========================================= shared variables initialization ===========================================*/
    ...
    if((has_captain_id = shmget(IPC_PRIVATE, sizeof(int), IPC_CREAT | 0666)) == -1){
        fprintf(stderr, "ERROR:\tCould not set has_captain_id.\n");
        free_all();
        exit(1);
    }

    if((hasCaptain = shmat(has_captain_id, NULL, 0)) == NULL){
        fprintf(stderr, "ERROR:\tCould not create hasCaptain.\n");
        free_all();
        exit(1);
    }
    ...
    *shared_counter += 1;
    ...
    if(is_captain){
        ...
        *unboarded += 1;
        ...
        //if 3 members left, the captain can leave asi well
        if(*unboarded == 3){
            sem_post(captain_unboarding);
            *unboarded = 0;
        }
        else sem_post(unboarding);
        //sem_post(mutex);
    }
...
void free_all(){
    ...
    shmctl(has_captain_id, IPC_RMID, NULL);
}
